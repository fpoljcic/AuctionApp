...
public interface ProductRepository extends JpaRepository<Product, UUID> {
    ...
    @Query(value = "SELECT pr.id, pr.name, pr.start_price startPrice, pr.description, p.url " +
    ...
    List<EssentialProductInfoResponse> getFeaturedRandomProducts();

    @Query(value = "SELECT pr.id, pr.name, pr.start_price startPrice, pr.description, p.url " +
                   "FROM product pr INNER JOIN photo p on pr.id = p.product_id " +
                   "WHERE start_date <= now() AND end_date > now() AND p.featured = true " +
                   "ORDER BY start_date DESC LIMIT 10", nativeQuery = true)
    List<EssentialProductInfoResponse> getNewProducts();

    @Query(value = "SELECT pr.id, pr.name, pr.start_price startPrice, pr.description, p.url " +
                   "FROM product pr INNER JOIN photo p on pr.id = p.product_id " +
                   "WHERE start_date <= now() AND end_date > now() AND p.featured = true " +
                   "ORDER BY end_date LIMIT 10", nativeQuery = true)
    List<EssentialProductInfoResponse> getLastProducts();
}
